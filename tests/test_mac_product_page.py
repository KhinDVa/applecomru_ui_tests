...
@pytest.fixture()
def page(browser):
    page = ProductPage(browser, links.MAC_PAGE)
    with allure.step(f'Открываем {links.MAC_PAGE}'):
        page.open()
    return page
...
def test_top_bar_btns_working(page, param_btn):
    ...
    with allure.step('Проверка что переход произошел на нужную страницу'):
        assert page.browser.current_url == param_btn[1], \
            f'Ожидался переход на {param_btn[1]}, но произошел переход ' \
...
LOCATORS_LIST = locators_list(MacPageLocators)
...
def test_subbar_btns_displayed(page, locator):
    with allure.step(f'Проверка отображения кнопки {locator}'):
        assert page.is_element_present(MacPageLocators.__getattribute__(MacPageLocators, locator)), \
...
def test_subbar_btns_working(page, locator):
    with allure.step(f'Производим клик по кнопке {locator}'):
        page.click_to(MacPageLocators.__getattribute__(MacPageLocators, locator))
    with allure.step('Проверяем, что произошел переход'):
        assert page.browser.current_url != links.MAC_PAGE, \
