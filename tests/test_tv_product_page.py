...
from pages.locators import TvPageLocators
...
@pytest.fixture()
def page(browser):
    page = ProductPage(browser, links.TV_PAGE)
    with allure.step(f'Открываем {links.TV_PAGE}'):
        page.open()
    return page
...
LOCATORS_LIST = locators_list(TvPageLocators)
...
def test_subbar_btns_displayed(page, locator):
    with allure.step(f'Проверка отображения кнопки {locator}'):
        assert page.is_element_present(TvPageLocators.__getattribute__(TvPageLocators, locator)), \
...
def test_subbar_btns_working(page, locator):
    with allure.step(f'Производим клик по кнопке {locator}'):
        page.click_to(TvPageLocators.__getattribute__(TvPageLocators, locator))
    with allure.step('Проверяем, что произошел переход'):
        assert page.browser.current_url != links.TV_PAGE, \
